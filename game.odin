...
random_normalized_vector::proc()->[2]f16 {
	return la.normalize([2]f16{f16(rand.float32())*2-1,f16(rand.float32())*2-1}) }
...
render_cell::proc(name:string,i,j:f16,extra_rotation:f16=0,direction:Compass=.NORTH,layer:f16=LAYER_ABOVE_SURFACE,flags:Cell_Flags_Register={}) {
	...
	hovered:=(!mouse_pressed(.MOUSE_LEFT))&&inside_board(&state.board,f16(i),f16(j))?in_rect(la.array_cast(state.cursor,f16),state.board.projected_cell_rects[int(i)][int(j)]):false
...
fish_tick::proc(fish:^Fish) {
	...
	// fish.direction=la.normalize(fish.direction+0.01*random_normalized_vector())
...
cells_distance::proc(i1,j1,i2,j2:i8)->i8 {
	return i8(math.round(la.length([2]f32{f32(i1-i2),f32(j1-j2)}))) }
	// return auto_cast math.sqrt_f16(f16((i1-i2)*(i1-i2)+(j1-j2)*(j1-j2))) }
	// return int(la.floor(la.length([2]f32{f32(i1),f32(j1)}-[2]f32{f32(i2),f32(j2)}))) }
...
render_menu::proc() {
	...
	for iterate_board(board,&i,&j) {
		...
		hovered:=in_rect(la.array_cast(state.cursor,f16),state.board.projected_cell_rects[int(i)][int(j)])
...
inverse_project_pos::proc(pos:[2]f16)->[2]f16 {
	p:[3]f32=la.inverse(state.view_matrix)*[3]f32{f32(pos.x),f32(pos.y),1}
	return [2]f16{f16(p.x),f16(p.y)} }
...
render_board::proc() {
	...
	for i in 1..<len(Entity_Kind) {
		...
		// DICK
		if state.marker_kind==entity_kind do render_rect({rect.pos,1.4*rect.size},{1,0,0,1},0.0,depth=0.55)
...
game_tick::proc() {
	...
	state.view_matrix=zoom_matrix(f32(state.view_zoom))*pan_matrix(la.array_cast(state.view_pan,f32))
	...
	if state.control_state.submenu!=.NONE {
		state.control_state.drag+=inverse_project_vector(la.array_cast(state.mouse_delta,f16)).y/64
...
init_view::proc() {
	...
	state.view_matrix=la.MATRIX3F32_IDENTITY
	state.view_pan={0,0} }
